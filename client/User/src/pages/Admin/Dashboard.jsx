...
import { useDashboardStatusQuery } from '../../redux/api/api'
import { Loader } from '../../component/Layout/Loaders'
    ...
    <AdminLayout>
      ...
      <div className='flex flex-col gap-4 sm:flex-row items-center m-2'>
        ...
        <div className='border mx-auto w-full sm:w-1/2 flex justify-center items-center shadow-md p-1 m-5 rounded-md max-w-96 relative'>
          <PillaMessage labels={["Single Chat", "Group Chat"]} value={[data.status.totalChats - data.status.groupChat, data.status.groupChat]} />
      ...
      <div className='flex flex-col sm:flex-row justify-around items-center'>
        <Widget title={"users"} value={data.status.totalUsers} icon={<PersonIcon></PersonIcon>}></Widget>
        <Widget title={"chats"} value={data.status.totalChats} icon={<ChatIcon />} />
        <Widget title={"messages"} value={data.status.totalMessages} icon={<MessagesIcon />} />
      </div>
    </AdminLayout>
  )
}
